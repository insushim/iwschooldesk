...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle('studentRecord:exportLogs', async () => {
    const result = await dialog.showSaveDialog({
      ...
      defaultPath: `학생기록_로그_${(()=>{const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`})()}.json`,
...
`
  ...
  ipcMain.handle('studentRecord:exportCsv', async () => {
    const result = await dialog.showSaveDialog({
      title: '학생 기록 CSV 저장',
      defaultPath: `학생기록_${(()=>{const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`})()}.csv`,
  ...
  ipcMain.handle('data:export', async () => {
    ...
    const result = await dialog.showSaveDialog({
      defaultPath: `schooldesk-backup-${(()=>{const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`})()}.json`,
      filters: [{ name: 'JSON', extensions: ['json'] }]
    })
